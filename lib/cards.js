...
/**
 * Create the 110-card deck WITHOUT shuffling.
 * Used by provably fair system which applies its own deterministic shuffle.
 */
export function createDeckUnshuffled() {
  _id = 1;
  const cards = [];

  // Reuse the same card creation logic but skip shuffle
  // We call createDeck internally and "unshuffle" by sorting by ID
  const deck = createDeck();
  // Sort by card ID to get deterministic base order
