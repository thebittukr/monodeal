...
function makeBot(name) {
  ...
  return { id, name, hand: [], assets: {}, bank: [], isBot: true };
}

export function createSoloRoom(roomId, hostName) {
  const hostId = genId();
  const room = {
    roomId,
    phase: "waiting",
  ...
  // Add 3 bots
  for (let i = 0; i < 3; i++) {
    room.players.push(makeBot(BOT_NAMES[i]));
  }
  startGame(room);
...
function botPlayTurn(room, botIdx) {
  ...
  while (room.playsThisTurn < room.maxPlays && !room.pendingAction && room.phase === "playing" && safetyLimit++ < 10) {
