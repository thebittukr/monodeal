...
export function processMove(room, playerId, move) {
  ...
  switch (card.type) {
    case "property": playProperty(room, playerIdx, card, cardIdx);     break;
...
// ─── Play Property ────────────────────────────────────────────────────────────
function playProperty(room, playerIdx, card, cardIdx) {
  const player = room.players[playerIdx];
  player.hand.splice(cardIdx, 1);
  if (!player.assets[card.color]) player.assets[card.color] = [];
  player.assets[card.color].push(card);
  room.playsThisTurn++;
  addLog(room, `${player.name} added ${card.name} (${COLORS[card.color].label}) to their properties.`);
}
