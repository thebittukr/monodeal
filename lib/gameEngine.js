...
function makePlayer(id, name, avatarId = 15, dateInfo = null, userId = null) {
  return { id, name, hand: [], assets: {}, bank: [], avatar: avatarId, dateInfo: dateInfo || null, userId: userId || null };
}
...
// ─── Bot Player Factory (100 bot pool with country flags) ─────────────────────
function _flag(code) {
  if (!code) return "";
  try { return String.fromCodePoint(...code.toUpperCase().split("").map(c => 0x1F1E6 + c.charCodeAt(0) - 65)); } catch { return ""; }
}

// Bot names that look like real player usernames — no country codes, no gamer tags
const BOT_POOL = [
  { name: "jake92",       country: "US", avatar: 87, personality: "aggressive" },
  { name: "mia.b",        country: "US", avatar: 18, personality: "strategic" },
  { name: "sarah_k",      country: "US", avatar: 42, personality: "aggressive" },
  { name: "cooper",       country: "US", avatar: 55, personality: "balanced" },
  { name: "arjun.dev",    country: "IN", avatar: 6,  personality: "strategic" },
  { name: "priya22",      country: "IN", avatar: 51, personality: "balanced" },
  { name: "rohit_s",      country: "IN", avatar: 33, personality: "aggressive" },
  { name: "lucas.m",      country: "BR", avatar: 71, personality: "aggressive" },
  { name: "gabi_r",       country: "BR", avatar: 39, personality: "balanced" },
  { name: "bruna",        country: "BR", avatar: 82, personality: "strategic" },
  { name: "sophie.w",     country: "GB", avatar: 66, personality: "balanced" },
  { name: "jackk",        country: "GB", avatar: 28, personality: "aggressive" },
  { name: "putri",        country: "ID", avatar: 22, personality: "strategic" },
  { name: "rizky_a",      country: "ID", avatar: 12, personality: "balanced" },
  { name: "carlos99",     country: "MX", avatar: 77, personality: "aggressive" },
  { name: "val.r",        country: "MX", avatar: 48, personality: "strategic" },
  { name: "yuki_m",       country: "JP", avatar: 91, personality: "strategic" },
  { name: "emre",         country: "TR", avatar: 44, personality: "aggressive" },
  { name: "max.b",        country: "DE", avatar: 63, personality: "strategic" },
  { name: "chima",        country: "NG", avatar: 15, personality: "aggressive" },
  { name: "minho",        country: "KR", avatar: 34, personality: "strategic" },
  { name: "anya_p",       country: "RU", avatar: 59, personality: "aggressive" },
  { name: "janella",      country: "PH", avatar: 26, personality: "strategic" },
  { name: "marc.d",       country: "PH", avatar: 73, personality: "aggressive" },
  { name: "sakura",       country: "JP", avatar: 45, personality: "aggressive" },
  { name: "omar.h",       country: "EG", avatar: 88, personality: "aggressive" },
  { name: "lina.t",       country: "VN", avatar: 31, personality: "strategic" },
  { name: "sofi_a",       country: "AR", avatar: 67, personality: "unpredictable" },
  { name: "can.y",        country: "TR", avatar: 19, personality: "strategic" },
  { name: "hannah.m",     country: "DE", avatar: 53, personality: "aggressive" },
];

export function pickRandomBots(count) {
  const shuffled = [...BOT_POOL].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, count);
}

// Bot dates — pulled from the actual seeded dates in DB
// ~40% chance of no date (like real players who haven't equipped one)
const BOT_DATES = [
  { name: "Strawberry", rarity: "common" },
  { name: "Lily Park", rarity: "common" },
  { name: "Pixel", rarity: "common" },
  { name: "Ruby Cole", rarity: "common" },
  { name: "Jasmine Santos", rarity: "rare" },
  { name: "Kai Nakamura", rarity: "rare" },
  { name: "Nova", rarity: "rare" },
  { name: "Ryder Cole", rarity: "epic" },
  { name: "Scarlett Blaze", rarity: "epic" },
  { name: "Zephyr", rarity: "epic" },
  { name: "Aurora", rarity: "legendary" },
  { name: "Ace Diamond", rarity: "legendary" },
  null, null, null, null, null, null, null, null, // ~40% no date
];

export function makeBot(botDef) {
  const id = "BOT_" + Math.random().toString(36).slice(2, 8).toUpperCase();
  const randomDate = BOT_DATES[Math.floor(Math.random() * BOT_DATES.length)];
  return {
    id,
    name: botDef.name,
    hand: [],
    ...
    bank: [],
    isBot: true,
    avatar: botDef.avatar,
    countryCode: botDef.country,
    personality: botDef.personality || "balanced",
    dateInfo: randomDate ? { name: randomDate.name, rarity: randomDate.rarity } : null,
  };
}
...
export function createSoloRoom(roomId, hostName, avatarId = 15) {
  ...
  // Pick 3 random bots from the pool (different every game)
  const bots = pickRandomBots(3);
...
// ─── Room Factory ─────────────────────────────────────────────────────────────
export function createRoom(roomId, hostName, maxPlayers = MAX_PLAYERS, avatarId = 15, dateInfo = null, userId = null) {
  const mp     = Math.min(Math.max(maxPlayers, MIN_PLAYERS), MAX_PLAYERS);
  const hostId = genId();
  return {
    roomId,
    phase: "waiting",
    players: [makePlayer(hostId, hostName, avatarId, dateInfo, userId)],
    deck: createDeck(),
    discard: [],
    turnIndex: 0,
    playsThisTurn: 0,
...
// ─── Join Room ────────────────────────────────────────────────────────────────
export function joinRoom(room, playerName, avatarId = 15, dateInfo = null, userId = null) {
  ...
  room.players.push(makePlayer(playerId, playerName, avatarId, dateInfo, userId));
...
/**
 * runBotTurns — DELAYED version.
 * Instead of executing bot moves instantly, we schedule them with a timestamp.
 * The move only executes when the next state poll happens AFTER the delay.
 * This makes bots feel human — they "think" for 2-8 seconds.
 */
export function runBotTurns(room, depth = 0) {
  ...
  // If a bot move is scheduled but not ready yet, wait
  if (room._botMoveAt && now < room._botMoveAt) {
    return room; // Not time yet — bot is "thinking"
  }

  // Timer expired = bot should act NOW
  const timerExpired = room._botMoveAt && now >= room._botMoveAt;
  if (timerExpired) {
    room._botMoveAt = null;
  }

  // Helper: schedule a bot delay and return (bot will act on next poll after delay)
  function scheduleBot(personality, actionType) {
    const delay = actionType === "playCard"
      ? getBotCardPlayDelay(personality, room.playsThisTurn + 1)
      : getBotDelay(personality, actionType);
    room._botMoveAt = now + delay;
  }
  ...
  if (pa) {
    ...
    if (pa.jsnChain) {
      ...
      if (room.players[waitIdx]?.isBot) {
        if (!timerExpired) { scheduleBot(room.players[waitIdx].personality || "balanced", "respond"); return room; }
    ...
    if (Array.isArray(pa.toIdxList)) {
      for (const idx of pa.toIdxList) {
        if (!pa.respondedList?.includes(idx) && room.players[idx]?.isBot) {
          if (!timerExpired) { scheduleBot(room.players[idx].personality || "balanced", "respond"); return room; }
    ...
    if (pa.toIdx !== undefined && room.players[pa.toIdx]?.isBot) {
      if (!timerExpired) { scheduleBot(room.players[pa.toIdx].personality || "balanced", "respond"); return room; }
  ...
  if (room.players[room.turnIndex]?.isBot) {
    if (!timerExpired) { scheduleBot(room.players[room.turnIndex].personality || "balanced", "playCard"); return room; }
...
export function sanitizeState(room, playerId) {
  const sanitized = {
    ...
    players: room.players.map((p) => {
      // Strip internal bot/system fields from ALL players (including self)
      const { isBot, personality, disconnected, disconnectedAt, originalId, userId, ...cleanPlayer } = p;
      if (cleanPlayer.id === playerId) return cleanPlayer;
      return { ...cleanPlayer, hand: cleanPlayer.hand.map(() => ({ id: "?", type: "hidden" })) };
    }),
  };

  // ── CRITICAL: Strip sensitive game data ─────────────────────────────────
  // Deck & discard contain full card data — exposing them lets players
  // see upcoming draws and reconstruct the game. Only send counts.
  sanitized.deckCount = sanitized.deck?.length ?? 0;
  sanitized.discardCount = sanitized.discard?.length ?? 0;
  delete sanitized.deck;
  delete sanitized.discard;
  ...
  delete sanitized._botMoveAt; // bot scheduling internals
  delete sanitized._completionProcessed;
