...
function makePlayer(id, name, avatarId = 15, dateInfo = null) {
  return { id, name, hand: [], assets: {}, bank: [], avatar: avatarId, dateInfo: dateInfo || null };
}
...
// ─── Room Factory ─────────────────────────────────────────────────────────────
export function createRoom(roomId, hostName, maxPlayers = MAX_PLAYERS, avatarId = 15, dateInfo = null) {
  const mp     = Math.min(Math.max(maxPlayers, MIN_PLAYERS), MAX_PLAYERS);
  const hostId = genId();
  return {
    roomId,
    phase: "waiting",
    players: [makePlayer(hostId, hostName, avatarId, dateInfo)],
    deck: createDeck(),
    discard: [],
    turnIndex: 0,
    playsThisTurn: 0,
...
// ─── Join Room ────────────────────────────────────────────────────────────────
export function joinRoom(room, playerName, avatarId = 15, dateInfo = null) {
  ...
  room.players.push(makePlayer(playerId, playerName, avatarId, dateInfo));
...
export function sanitizeState(room, playerId) {
  const sanitized = {
    ...
    players: room.players.map((p) => {
      ...
      const { isBot, personality, disconnected, disconnectedAt, originalId, ...cleanPlayer } = p;
