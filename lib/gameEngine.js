...
export function handleResponse(room, playerIdx, move) {
  ...
  // ── Property-payment response (payer selects asset cards) ────────────────
  if (pending.type === "property-payment") {
    if (pending.toIdx !== playerIdx) throw new Error("This payment is not directed at you");
    return handlePropertyPayment(room, playerIdx, move);
  }
  ...
  if (isMulti) {
    ...
    if (move.response === "block") {
      ...
      const queue = [];
      applySingleTarget(room, pending, playerIdx, queue);
      pending.respondedList.push(playerIdx);
      // Collect any property-payment debts into the master queue
      if (!pending.propertyPaymentQueue) pending.propertyPaymentQueue = [];
      pending.propertyPaymentQueue.push(...queue);
    }

    // All targets responded → resolve or start property-payment chain
    if (pending.respondedList.length >= pending.toIdxList.length) {
      const queue = pending.propertyPaymentQueue ?? [];
      room.pendingAction = queue.length > 0 ? nextPropertyPayment(queue) : null;
    }
  ...
  if (move.response === "block") {
    ...
    const queue = [];
    applyPendingAction(room, pending, queue);
    room.pendingAction = queue.length > 0 ? nextPropertyPayment(queue) : null;
  }
...
// ─── Build next property-payment pendingAction from queue ─────────────────────
function nextPropertyPayment(queue) {
  const next = queue.shift();
  return {
    type: "property-payment",
    fromIdx: next.fromIdx,
    toIdx:   next.toIdx,
    params:  { remaining: next.remaining },
    queue,   // remaining items after this one
  };
}

// ─── Handle a payer selecting property cards to cover remaining debt ──────────
function handlePropertyPayment(room, playerIdx, move) {
  const pending  = room.pendingAction;
  const payer    = room.players[playerIdx];
  const receiver = room.players[pending.fromIdx];
  const { remaining } = pending.params;
  const { cards = [] } = move; // [{ color, cardId }, ...]

  // Validate & collect chosen property cards
  let valueGiven = 0;
  const given = [];
  for (const { color, cardId } of cards) {
    const arr = payer.assets[color] ?? [];
    const idx = arr.findIndex((c) => c.id === cardId);
    if (idx === -1) throw new Error(`Card ${cardId} not found in your ${color} assets`);
    const [card] = arr.splice(idx, 1);
    given.push({ card, color });
    valueGiven += card.value ?? 0;
  }

  const allAssets = Object.values(payer.assets).flat();
  const hasMoreAssets = allAssets.length > 0;

  // Must either cover the debt or have given everything
  if (valueGiven < remaining && hasMoreAssets) {
    // Roll back — restore taken cards
    given.forEach(({ card, color }) => {
      if (!payer.assets[color]) payer.assets[color] = [];
      payer.assets[color].push(card);
    });
    throw new Error(`Select more properties — still need $${remaining - valueGiven}M, or give all remaining assets`);
  }

  // Place given cards into receiver's assets
  given.forEach(({ card, color }) => {
  ...
  if (given.length > 0) {
    addLog(room, `${payer.name} gave ${given.length} propert${given.length > 1 ? "ies" : "y"} ($${valueGiven}M) to ${receiver.name} as payment.`);
  } else {
    addLog(room, `${payer.name} has nothing left to give — debt forgiven.`);
  }
...
// ─── Queue a property-payment if bank couldn't cover the debt ─────────────────
function maybeQueuePropertyPayment(room, attacker, defender, fromIdx, toIdx, amount, paid, stillOwed, queue) {
  attacker.bank.push(...paid);
  if (stillOwed > 0) {
    const hasAssets = Object.values(defender.assets).flat().length > 0;
    if (hasAssets) {
      queue.push({ fromIdx, toIdx, remaining: stillOwed });
      addLog(room, `${defender.name} paid $${amount - stillOwed}M cash but still owes $${stillOwed}M in properties!`);
    } else {
      addLog(room, `${defender.name} is broke — paid what they could ($${amount - stillOwed}M).`);
    }
  } else {
    addLog(room, `${defender.name} paid $${amount - stillOwed}M to ${attacker.name}.`);
  }
}

// ─── Apply single-target action to one player (for multi-target) ─────────────
function applySingleTarget(room, pending, targetIdx, queue = []) {
  ...
  if (pending.type === "rent" || pending.type === "birthday" || pending.type === "debtcollector") {
    const { paid, total, stillOwed } = collectPayment(defender, params.amount);
    maybeQueuePropertyPayment(room, attacker, defender, pending.fromIdx, targetIdx, params.amount, paid, stillOwed, queue);
  } else if (pending.type === "propertytax") {
    const amount = countCompletedSets(defender.assets);
    if (amount > 0) {
      const { paid, stillOwed } = collectPayment(defender, amount);
      maybeQueuePropertyPayment(room, attacker, defender, pending.fromIdx, targetIdx, amount, paid, stillOwed, queue);
    } else {
...
function applyPendingAction(room, pending, queue = []) {
  const attacker = room.players[pending.fromIdx];
  ...
  switch (pending.type) {
    case "debtcollector":
    case "birthday": {
      const { paid, stillOwed } = collectPayment(defender, params.amount);
      maybeQueuePropertyPayment(room, attacker, defender, pending.fromIdx, pending.toIdx, params.amount, paid, stillOwed, queue);
      break;
    }
    ...
    case "taxtherich": {
      ...
      if (amount > 0) {
        const { paid, stillOwed } = collectPayment(defender, amount);
        maybeQueuePropertyPayment(room, attacker, defender, pending.fromIdx, pending.toIdx, amount, paid, stillOwed, queue);
      } else {
...
// ─── Collect Payment ──────────────────────────────────────────────────────────
// Greedy: always take the largest card that fits (≤ remaining).
// If no card fits (all remaining cards exceed the debt), take the smallest
// available card so we overpay by the minimum possible amount.
function collectPayment(player, amount) {
  let remaining = amount;
  const paid = [];
  const bank = [...player.bank].sort((a, b) => b.value - a.value); // desc

  while (remaining > 0 && bank.length > 0) {
    // findIndex on descending array → first (largest) card that doesn't overshoot
    let idx = bank.findIndex((c) => c.value <= remaining);
    // No exact-fit card available → take the smallest card to minimise overpay
  ...
  const stillOwed = Math.max(0, amount - total);
  return { paid, total, stillOwed };
}
...
function advanceTurn(room, playerIdx) {
  ...
  addLog(room, `${current.name} ended their turn. ${next.name} drew ${drawn} cards.`);
