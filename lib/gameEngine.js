...
export function runBotTurns(room, depth = 0) {
  ...
  // Clear the scheduled time (we're about to execute)
  if (room._botMoveAt) {
  ...
  if (pa) {
    ...
    if (pa.jsnChain) {
      ...
      if (room.players[waitIdx]?.isBot) {
        if (!room._botMoveAt) {
          // Schedule with delay (3-6s for responses)
          const personality = room.players[waitIdx].personality || "balanced";
          const delay = getBotDelay(personality, "respond");
          room._botMoveAt = now + delay;
          return room; // Wait for next poll
        }
    ...
    if (Array.isArray(pa.toIdxList)) {
      for (const idx of pa.toIdxList) {
        if (!pa.respondedList?.includes(idx) && room.players[idx]?.isBot) {
          if (!room._botMoveAt) {
            const personality = room.players[idx].personality || "balanced";
            const delay = getBotDelay(personality, "respond");
            room._botMoveAt = now + delay;
            return room;
          }
          botRespondToAction(room, idx);
          room._botMoveAt = null; // Reset for next bot
          return runBotTurns(room, depth + 1);
        }
      }
      return room;
    ...
    if (pa.toIdx !== undefined && room.players[pa.toIdx]?.isBot) {
      if (!room._botMoveAt) {
        const personality = room.players[pa.toIdx].personality || "balanced";
        const delay = getBotDelay(personality, "respond");
        room._botMoveAt = now + delay;
        return room;
      }
  ...
  if (room.players[room.turnIndex]?.isBot) {
    if (!room._botMoveAt) {
      // Schedule the bot's move with personality-based delay
      const bot = room.players[room.turnIndex];
      const personality = bot.personality || "balanced";
      const cardNum = room.playsThisTurn + 1;
      const delay = getBotCardPlayDelay(personality, cardNum);
      room._botMoveAt = now + delay;
      return room; // Bot is "thinking" — wait for next poll
    }
