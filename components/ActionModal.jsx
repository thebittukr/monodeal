    ...
    <div className="flex flex-col gap-3">
      ...
      {allCash.length > 0 && (
        <div className="flex gap-2">
          <button
            onClick={() => {
              // Auto-select minimum cash cards to cover the amount
              let running = 0;
              const picks = [];
            ...
            className="flex-1 py-2 rounded-lg bg-amber-600/20 border border-amber-500/30 text-amber-400 text-[10px] font-bold hover:bg-amber-600/30 transition active:scale-95"
          >
            Quick Pay (Cash Only)
          </button>
          <button
            onClick={() => { setSelCash(allCash.map(c => c.id)); setSelProps([]); }}
            className="flex-1 py-2 rounded-lg bg-slate-700/50 border border-white/10 text-slate-400 text-[10px] font-bold hover:bg-slate-700 transition active:scale-95"
          >
            Select All Cash
          </button>
        </div>
      ...
      <button
        onClick={() => { setPaying(true); onConfirm({ cashIds: selCash, cards: selProps }); }}
        disabled={paying || (!canConfirm && (allCash.length > 0 || allProps.length > 0))}
        className="w-full py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-600 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold transition-all active:scale-95"
      >
        {paying ? "Paying..."
          : allCash.length === 0 && allProps.length === 0
          ? "OK — Nothing to Give"
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2">
        {hasJustSayNo && (
          <button
            disabled={responding}
            onClick={() => { setResponding(true); onBlock(); }}
            className="w-full py-3.5 rounded-xl bg-red-600 hover:bg-red-500 disabled:opacity-60 text-white font-bold transition-all flex items-center justify-center gap-2 active:scale-95"
          >
            {responding ? "Blocking..." : <><span>🚫</span><span>Just Say No!</span></>}
          </button>
        )}
        <button
          disabled={responding}
          onClick={() => { setResponding(true); onAccept(); }}
          className="w-full py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-bold transition-all active:scale-95"
        >
