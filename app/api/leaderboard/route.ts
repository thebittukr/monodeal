 ...
 * Cached in-memory for 2 minutes (data barely changes).
 */
...
export async function GET(req: Request) {
  ...
  try {
    // Cache the raw data for 2 minutes — sorting/slicing is cheap
    const { combined, popularGirlfriends } = await cached(
      "leaderboard:data",
      120_000, // 2 min
      async () => {
        // Real players
        ...
        return { combined, popularGirlfriends };
      }
    );

    // Sort (cheap — cached array is already in memory)
    ...
    const rankings = sorted.slice(0, limit).map((p, i) => ({ ...p, rank: i + 1 }));
